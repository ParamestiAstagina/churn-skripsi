...
def clean_dataset(df: pd.DataFrame, has_target: bool = True):
    """
    Membersihkan dataset sesuai preprocessing notebook:
    1. Menghapus CLIENTNUM.
    2. Menghapus dua kolom Naive Bayes.
    3. Mapping target:
       Existing Customer = 0
       Attrited Customer = 1
    """
...
def get_recommendation(risk):
    if risk == "Rendah":
        return (
            "Nasabah memiliki risiko churn rendah. Pertahankan layanan, "
            "berikan promosi ringan, dan jaga engagement secara berkala."
        )

    if risk == "Sedang":
        return (
            "Nasabah perlu dipantau. Berikan penawaran cashback, reminder benefit kartu, "
            "atau program loyalitas sederhana."
        )

    return (
        "Nasabah berisiko tinggi churn. Prioritaskan untuk program retensi, "
        "follow-up customer care, evaluasi keluhan, dan tawarkan benefit khusus."
    )
...
def run_bayesian_optimization(X_train, y_train, cat_features, init_points=3, n_iter=10):
    try:
        ...
        raise ImportError(
            "Package bayesian-optimization belum terinstall. "
            "Tambahkan bayesian-optimization di requirements.txt."
        ) from exc
    ...
    def cv_f2(iterations, learning_rate, depth, l2_leaf_reg):
        ...
        for train_idx, val_idx in cv.split(X_train, y_train):
            ...
            probability = model.predict_proba(X_va)[:, 1]
            prediction = (probability >= 0.50).astype(int)
            score = fbeta_score(y_va, prediction, beta=2, pos_label=1)
...
def find_best_threshold(X_train, y_train, cat_features, best_params):
    ...
    for i, threshold in enumerate(thresholds):
        ...
        status.write(f"Mencari threshold terbaik: {threshold:.2f}")
...
def train_optimized_model(X_train, y_train, X_val, y_val, cat_features, best_params):
    model = CatBoostClassifier(
        auto_class_weights="Balanced",
        random_seed=42,
        verbose=0,
...
if menu == "Dashboard":
    st.markdown(
        '<p class="main-header">Prediksi Churn Nasabah Kartu Kredit</p>',
        unsafe_allow_html=True,
    )
    st.markdown(
    ...
    if raw_df is None:
        st.error("Dataset default belum ditemukan. Letakkan BankChurners.csv di folder data/.")
        st.stop()
    ...
    col1.metric("Total Nasabah", f"{len(df_clean):,}")
    col2.metric("Nasabah Churn", f"{churn_count:,}")
    col3.metric("Nasabah Tidak Churn", f"{stay_count:,}")
...
# ======================================================
    ...
    tab1, tab2, tab3, tab4 = st.tabs(
        ["Struktur Data", "Missing & Duplikat", "Visualisasi", "Korelasi"]
    )
        ...
        info_df = pd.DataFrame(
            {
                ...
                "Tipe Data": [str(df_clean[col].dtype) for col in df_clean.columns],
                "Jumlah Unik": [df_clean[col].nunique() for col in df_clean.columns],
            }
        )
        ...
        with col2:
            ...
            for col in df_clean.select_dtypes(include="object").columns:
                unknowns.append(
                    {
                        "Kolom": col,
                        "Jumlah Unknown": int((df_clean[col] == "Unknown").sum()),
                    }
                )
    ...
    with tab3:
        target_label = df_clean[TARGET_COL].map(
            {
                0: "Existing Customer",
                1: "Attrited Customer",
            }
        ...
        fig = px.histogram(
            ...
            color="Status",
            marginal="box",
            title=f"Distribusi {selected_num} terhadap Churn",
        )
        st.plotly_chart(fig, use_container_width=True)
        ...
        fig = px.bar(
            ...
            color="Status",
            barmode="group",
            title=f"{selected_cat} terhadap Churn",
        )
        st.plotly_chart(fig, use_container_width=True)
...
# ======================================================
    ...
    st.info(
        "Halaman ini mengikuti alur notebook: seleksi atribut, transformasi target, "
        "train-test split, CatBoost baseline, Bayesian Optimization, optimasi threshold, "
        "lalu training model final."
    )
    ...
    with st.expander("Pengaturan training", expanded=True):
        ...
        run_threshold_search = st.checkbox(
    ...
    if st.button("🚀 Jalankan Training & Optimasi", type="primary"):
        ...
        st.dataframe(
            pd.DataFrame([baseline_metrics]).T.rename(columns={0: "Score"}),
        ...
        if not bo_history.empty:
            ...
            fig = px.line(
                bo_history.reset_index(),
                x="index",
                y="target",
                markers=True,
                title="Perkembangan Skor F2 pada Bayesian Optimization",
            )
            st.plotly_chart(fig, use_container_width=True)
        ...
        if run_threshold_search:
            ...
            fig = px.line(
                ...
                x="threshold",
                y="f2_score",
                title="F2-score Berdasarkan Threshold",
            )
            st.plotly_chart(fig, use_container_width=True)
        else:
            best_threshold = 0.50
        ...
        with st.spinner("Training model final CatBoost + Bayesian Optimization..."):
            bo_model = train_optimized_model(
                X_tr_final,
                y_tr_final,
                X_val_final,
                y_val_final,
            ...
            bo_metrics, y_pred_bo, y_prob_bo = evaluate_model(
                bo_model,
                X_test,
                y_test,
        ...
        st.success("Model final berhasil dilatih dan disimpan sementara di session Streamlit.")
...
# ======================================================
    ...
    with col1:
        ...
        fig = px.imshow(
            ...
            title="Confusion Matrix CatBoost + Bayesian Optimization",
        )
        st.plotly_chart(fig, use_container_width=True)
...
# ======================================================
    ...
    columns_area = [col1, col2, col3]

    for i, col in enumerate(feature_columns):
        with columns_area[i % 3]:
    ...
    if st.button("Prediksi Churn", type="primary"):
        ...
        col1, col2, col3 = st.columns(3)
        col1.metric("Prediksi", "Churn" if prediction == 1 else "Tidak Churn")
        col2.metric("Probabilitas Churn", f"{probability * 100:.2f}%")
        col3.metric("Kategori Risiko", risk)
...
# ======================================================
    ...
    uploaded = st.file_uploader(
        "Upload data nasabah baru CSV/XLSX",
        type=["csv", "xlsx", "xls"],
    )
    ...
    if uploaded is not None:
        ...
        probabilities = model.predict_proba(batch_X)[:, 1]
        predictions = (probabilities >= threshold).astype(int)

        result = batch_raw.copy()
        result["Probabilitas_Churn"] = probabilities
        result["Probabilitas_Churn_%"] = probabilities * 100
        result["Prediksi"] = np.where(predictions == 1, "Churn", "Tidak Churn")
        result["Kategori_Risiko"] = [get_risk_category(prob) for prob in probabilities]
        result["Rekomendasi"] = [get_recommendation(risk) for risk in result["Kategori_Risiko"]]

        st.subheader("Hasil Prediksi")
        st.dataframe(result, use_container_width=True)

        col1, col2, col3 = st.columns(3)
        col1.metric("Total Data", len(result))
        col2.metric("Prediksi Churn", int((result["Prediksi"] == "Churn").sum()))
        col3.metric(
            "Rata-rata Probabilitas Churn",
            f"{result['Probabilitas_Churn_%'].mean():.2f}%",
        )
